...
int	valid_move(t_data *data, int move, float x, float y)
{
	if (move == 0)
	{
		x = data->player_x + data->dir_x * 0.3;
		y = data->player_y + data->dir_y * 0.3;
	}
	else if (move == 1)
	{
		x = data->player_x - data->dir_x * 0.3;
	...
	{
		x = data->player_x + data->dir_y * 0.3;
		y = data->player_y - data->dir_x * 0.3;
	}
	else if (move == 3)
	{
		x = data->player_x - data->dir_y * 0.3;
...
void	move_player2(t_data *data, int move)
{
	if (move == 2 && valid_move(data, 2, 0, 0))
	{
...
int	mouse_move(int x, int y, t_data *data)
{
	...
	if (delta_x != 0)
	{
		data->dir_x += -data->dir_y * (PI / 8)
			* ((float)delta_x / (SCREEN_WIDTH / 2));
		data->dir_y += data->dir_x * (PI / 8)
			* ((float)delta_x / (SCREEN_WIDTH / 2));
		data->plane_x = -data->dir_y * 0.66;
		data->plane_y = data->dir_x * 0.66;
		data->check_move = 2;
